...
function togglePassword(element) {
    // Find the password input - it could be a sibling or in parent container
    const container = element.closest('.flex, .relative');
...
// Debug: Log API base and current user
...
async function handleSignup(event) {
    ...
    try {
        ...
        // Fallback: Create account locally if server is offline
        if (error.message.includes('fetch') || error.message.includes('NetworkError')) {
            console.log('Server offline, creating account locally');
            const newUser = {
            ...
            button.textContent = 'Account Created (Offline)!';
            setTimeout(() => {
                window.location.href = 'homepage.html';
            }, 1000);
        } else {
...
async function handleLogin(event) {
    ...
    try {
        ...
        if (result.success) {
            ...
            localStorage.removeItem('cleanbite_form_form'); // Clear saved form data
...
async function handleBooking(event) {
    ...
    const guestCount = document.querySelector('input[type="number"]').value;
    const requirements = document.querySelector('textarea.form-input-refined').value;
    
    if (!eventType || !date || !location || !guestCount) {
        button.innerHTML = originalText;
        button.disabled = false;
        alert('Please fill in all required fields including guest count');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            const userBookings = JSON.parse(localStorage.getItem('cleanbite_user_bookings') || '[]');
            userBookings.push({
                vendorName: vendorData.name,
                eventType,
                date,
            ...
            localStorage.setItem('cleanbite_user_bookings', JSON.stringify(userBookings));
            ...
            const fields = [document.querySelector('select.form-input-refined'), document.querySelector('input[type="date"]'), document.querySelector('input[placeholder*="Stanford"]'), document.querySelector('input[type="number"]'), document.querySelector('textarea.form-input-refined')];
                    ...
                    ${bookings.map(booking => `
                        <div class="p-4 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100 dark:border-slate-800">
                            ...
                            <div class="grid grid-cols-2 gap-4 text-sm text-slate-600 dark:text-slate-400">
                                ...
                                <p class="col-span-1"><strong>Location:</strong> ${booking.location}</p>
...
function showBookingsModal(bookings) {
    ...
    const bookingsList = bookings.map(booking => `
        <div class="bg-white dark:bg-[#1c2620] p-4 rounded-lg border border-[#dee3e0] dark:border-[#2d3a31] mb-3">
            ...
            <p class="text-sm text-[#6b8072] mb-1"><strong>Guests:</strong> ${booking.guestCount || 'Not specified'}</p>
...
async function handleReport(event) {
    ...
    try {
        ...
        if (result.success) {
            ...
            localStorage.removeItem('cleanbite_report_form');
            
            // Save report to localStorage as backup
            ...
            // Get selected vendor to redirect to vendor profile
            ...
            setTimeout(() => {
                ...
                if (selectedVendor) {
                    window.location.href = `vendor detailpage.html?vendor=${selectedVendor}`;
                } else {
...
async function loadUserReports() {
    ...
    try {
        const response = await fetch(`${API_BASE}/reports/${currentUser.id}`);
        const reports = await response.json();
        
        const reportCards = document.querySelectorAll('.group.cursor-pointer.flex.flex-col.gap-4');
        const reportCard = reportCards[2]; // Third card is reports
        if (reportCard && reports.length > 0) {
            const latestReport = reports[reports.length - 1];
            const reportText = reportCard.querySelector('p');
            if (reportText) {
                reportText.textContent = `${latestReport.issueType} - ${latestReport.vendorId}`;
            }
            
            const statusBadge = reportCard.querySelector('span');
            if (statusBadge) {
                statusBadge.textContent = reports.length + ' Total';
            }
            
            reportCard.onclick = () => showReportsModal(reports);
        }
    } catch (error) {
...
function showReportsModal(reports) {
    ...
    const reportsList = reports.map(report => `
        <div class="bg-white dark:bg-[#1c2620] p-4 rounded-lg border border-[#dee3e0] dark:border-[#2d3a31] mb-3">
            <div class="flex justify-between items-start mb-2">
                <h4 class="font-bold text-[#131614] dark:text-white">${report.issueType}</h4>
                <span class="px-2 py-1 bg-orange-100 text-orange-800 text-xs font-bold rounded">${report.status}</span>
            </div>
            <p class="text-sm text-[#6b8072] mb-1"><strong>Vendor:</strong> ${report.vendorId}</p>
            <p class="text-sm text-[#6b8072] mb-1"><strong>Date:</strong> ${new Date(report.createdAt).toLocaleDateString()}</p>
            <p class="text-sm text-[#6b8072]"><strong>Description:</strong> ${report.description}</p>
        </div>
    `).join('');
    ...
    modal.innerHTML = `
        <div class="bg-white dark:bg-[#1c2620] rounded-xl w-full max-w-2xl max-h-[80vh] overflow-auto">
            <div class="flex items-center justify-between p-6 border-b border-[#dee3e0] dark:border-[#2d3a31]">
                <h3 class="text-xl font-bold text-[#131614] dark:text-white">My Reports</h3>
                <button onclick="this.closest('.fixed').remove()" class="text-[#6b8072] hover:text-[#131614] dark:hover:text-white">
                    <span class="material-symbols-outlined">close</span>
                </button>
            </div>
            <div class="p-6">
                ${reportsList || '<p class="text-center text-[#6b8072]">No reports found</p>'}
            </div>
        </div>
    `;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.location.pathname.includes('booking.html')) {
        ...
        bookingInputs.forEach((input, index) => {
            ...
            input.addEventListener('input', () => {
                const bookingData = {
                    ...
                    guestCount: document.querySelector('input[type="number"]').value,
        ...
        if (savedBooking) {
            ...
            if (savedBooking.guestCount) document.querySelector('input[type="number"]').value = savedBooking.guestCount;
...
function loadRecentActivities() {
    ...
    // Get recent bookings
    const userBookings = JSON.parse(localStorage.getItem('cleanbite_user_bookings') || '[]');
    userBookings.slice(-3).forEach(booking => {
        activities.push({
            type: 'booking',
            ...
            description: `${booking.vendorName} - ${booking.eventType}`,
    ...
    // Get recent reports
    const userReports = JSON.parse(localStorage.getItem('cleanbite_user_reports') || '[]');
    userReports.slice(-3).forEach(report => {
        activities.push({
            type: 'report',
            icon: 'report_problem',
            iconBg: 'bg-amber-100',
            ...
            description: `${report.issueType} - ${report.vendorId}`,
    ...
    if (activities.length === 0) {
        activitiesContainer.innerHTML = '<div class="text-center text-[#6b8072] py-8">No recent activities</div>';
        return;
    }
    
    const activitiesHtml = activities.slice(0, 5).map(activity => `
        <div class="flex items-center gap-4 p-4 hover:bg-background-light dark:hover:bg-[#243128] transition-colors">
            <div class="size-10 ${activity.iconBg} rounded-full flex items-center justify-center ${activity.iconColor}">
                <span class="material-symbols-outlined text-[20px]">${activity.icon}</span>
            </div>
            <div class="flex-1">
                <p class="text-sm font-bold text-[#131614] dark:text-white">${activity.title}</p>
                <p class="text-xs text-[#6b8072]">${activity.description}</p>
            </div>
            <p class="text-xs text-[#6b8072]">${new Date(activity.time).toLocaleDateString()}</p>
        </div>
    `).join('');
